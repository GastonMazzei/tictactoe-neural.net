...
def create_model_three_classes(s,NEUR=8, name="LSTM_chess_model", ACT='tanh'):
	inputs = tf.keras.Input(shape=s)
	lstm = tf.keras.layers.LSTM(NEUR, )#return_sequences=False, return_state=False)
	x = lstm(inputs)
...
def create_model_nine_classes(s,NEUR=8, name="LSTM_chess_model"):
	inputs = tf.keras.Input(shape=s)
	lstm = tf.keras.layers.LSTM(NEUR, )#return_sequences=False, return_state=False)
	x = lstm(inputs)
	x = tf.keras.layers.Dense(NEUR, activation="relu")(x)
...
if __name__=='__main__':
	...
	model.save('model-onlywin2.h5', save_format="h5")
